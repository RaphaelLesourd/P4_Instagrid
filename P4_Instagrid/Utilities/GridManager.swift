...
class GridManager {
    ...
    // MARK: - UIView Converter
    /// Convert any view to an image
    /// - Parameters:
    ///   - view: Pass in a view to convert
    ///   - completion: Sends back an image
    ...
    ///
    /// - This method first checks how many images are availlable by checking
    /// the isHidden attribute of each views in the stackView.
    /// A count of available image is increased.
    /// - Then each visible button is checked to determine if its image is the emptyState image (+)
    ///  or anther image. If the image in not the emptyState image then the button is considered
    ///  changed with a selected image and a count is increased by 1.
    ...
    func gridComplete(for stackView: UIStackView, imageToCheck: UIImage) -> Bool {
        ...
        for view in stackView.arrangedSubviews {
            ...
            /// if the button (view) is not hiden and the image in the button is
            /// not the empty state image (image has been replaced)
